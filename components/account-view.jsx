      ...
      {upcoming.length > 0 ? (
        <div className="mb-9 flex flex-col gap-[14px]">
          {upcoming.map((t) => (
            <Link
              key={t.txnId}
              href={`/tickets/${t.txnId}`}
              className="flex items-center gap-5 rounded-[14px] border border-[#eee] p-5 transition-colors hover:border-accent-border"
            >
              <div
        ...
        <div className="mb-9 rounded-[14px] border border-[#eee] p-10 text-center text-[14px] text-fainter">
          ยังไม่มีตั๋ว —{" "}
        ...
        <>
          ...
          <div className="flex flex-col gap-[14px]">
            {past.map((t) => (
              <div
                key={t.txnId}
                className="flex items-center gap-5 rounded-[14px] border border-[#f0f0f1] p-5 opacity-65"
              >
                <div
                  className="aspect-[3/4] w-16 flex-shrink-0 rounded-[8px]"
                  style={{ background: t.grad }}
