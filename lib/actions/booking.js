...
// แปลงข้อความ error จาก RPC (เช่น 'SEAT_TAKEN: ที่นั่ง A-5 ไม่ว่างแล้ว') เป็นไทย
function toThaiError(message) {
  ...
  return map[raw] || "ทำรายการไม่สำเร็จ กรุณาลองใหม่";
}
...
export async function holdSeatsAction(seatIds) {
  ...
  if (error) return { ok: false, error: toThaiError(error.message) };
