...
// ยกเลิกออเดอร์ (กดย้อนกลับ / ออกจาก checkout / หมดเวลา) -> คืนที่นั่ง/โควต้า
// ยกเลิกได้เฉพาะออเดอร์ของตัวเองที่ยัง 'pending' เท่านั้น
// (กัน watcher เผลอยกเลิกออเดอร์ที่จ่ายเงินไปแล้วช่วงเปลี่ยนหน้าไป /tickets)
export async function cancelOrderAction(txnId) {
  ...
  if (!txn || txn.buyer_id !== buyerId)
    return { ok: false, error: "ไม่พบออเดอร์" };
  if (txn.status !== "pending") return { ok: true }; // จ่าย/ยกเลิกไปแล้ว = ไม่ทำอะไร
