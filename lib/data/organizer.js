...
export async function getOrganizerEventReport(eventId, scope) {
  ...
  const { data: e } = await db
    .from("events")
    .select("id,title,status,organizer_id,ticket_types(id,name,price,quantity_total)")
    .eq("id", eventId)
    .maybeSingle();
  if (!e) return null;
...
// ---- ข้อมูลอีเวนต์สำหรับหน้าแก้ไข (ownership-checked) ----
export async function getOrganizerEventForEdit(eventId, scope) {
  if (!eventId || !UUID_RE.test(eventId)) return null;
  const db = supabaseServer();
  ...
  const { data: e } = await db
    .from("events")
    .select(
      "id,title,description,category,starts_at,status,organizer_id,venue_id," +
        "venues(name),ticket_types(id,name,price,seating_type,quantity_total,quantity_sold,seat_rows,seat_cols)"
    )
    .eq("id", eventId)
    .maybeSingle();
  if (!e) return null;
  if (scope && e.organizer_id !== scope) return null;
  // อีเวนต์ที่ถูกยกเลิกแล้ว แก้ไขไม่ได้ -> ปิดหน้าแก้ไข (notFound)
  ...
  return {
    ...
    zones: (e.ticket_types || [])
      ...
      .map((t) => ({
        ...
        type: t.seating_type,
        capacity: Number(t.quantity_total),
        rows: t.seat_rows ?? "",
        cols: t.seat_cols ?? "",
        sold: Number(t.quantity_sold),
      })),
  };
}
